...
package object nio {
  ...
  implicit class FileOps(file: File){
    ...
    def write(in: InputStream) = FileUtils.write(file, in)
    def writeTo(out: OutputStream) = FileUtils.writeTo(file, out)
    ...
    def foreachLines(charset: String = "UTF-8")(f: String => Unit) = FileUtils.foreachLines(file, charset)(f)
    def foreachBytes(chunkSize: Int = 1024 * 8)(f: Array[Byte] => Unit) = FileUtils.foreachBytes(file, chunkSize)(f)
...
//    def bytes(chunkSize: Int = 1024 * 8) = FileUtils.readBytes(file, chunkSize)
  ...
  implicit class InputStreamOps(in: InputStream){
    def readAsBytes() = StreamUtils.readAsBytes(in)
    def readAsString(charset: String = "UTF-8") = StreamUtils.readAsString(in, charset)
    def writeTo(file: File) = FileUtils.write(file, in)
    def writeTo(out: OutputStream) = StreamUtils.transfer(in, out)
  ...
  implicit class OutputStreamOps(out: OutputStream){
    def writeFrom(file: File) = FileUtils.writeTo(file, out)
    def writeFrom(in: InputStream) = StreamUtils.transfer(in, out)
